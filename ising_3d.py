...
def sim_tt(N, tt, b, imp):
    ...
    for i in range(eqSteps):  # equilibrate
        mcmove(config, beta, b)  # Monte Carlo moves
    for i in range(mcSteps):
        mcmove(config, beta, b)
        ...
        M2 += (n1 * Mag * Mag)
        E2 += (n1 * Ene * Ene)
    return n1 * E1, n1 * M1, (E2 - n2 * E1 * E1) * (beta * beta), (M2 - n2 * M1 * M1) * beta
...
if __name__ == "__main__":
    ...
    plt.savefig(f'ising_3d_MC_test_N={str(N)}_{"OPA"[b]}_{"YN"[imp]}.png', bbox_inches='tight', dpi=300)
    for i in range(4):
        letterr = 'EMCX'[i]
